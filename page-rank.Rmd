...
One of the requirements of using a Markov chain model is a matrix whose columns sum to 1. Here is an example of a matrix which does not.
...
What happens if we vary the single hub example so that the hub points only to another page instead of itself? As shown, the hub has the largest PageRank, but the page it points to has nearly as much as the hub "passes" its PageRank to the other page.
