...
In the above example you see the impact of the damping factor in force. What if we ran the Markov Chain without the damping factor? As you can see below, the entire weight of probability in the steady state is placed on the single hub. This is true even though we started with an even probability of starting on each page.

```{r hub-no-damping}
markov.demo(graph, initial, damping.factor=1, print.skip = 5)
```
...
What if we were to attempt to run our Markov chain on the above example without correcting for the dangling node, Page A  which has no outgoing links?

```{r dual-dangling}
markov.demo(graph, initial, print.skip = 20, fix.dangling = FALSE)
...
markov.demo(graph, initial, damping.factor = 1, print.skip = 250)
